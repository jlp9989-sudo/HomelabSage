...
- The schema only applies to `analyze()`; the curator's `generate_text()`
  ...
  field-for-field.
- **Note provenance** (`notes.py` + `prompts/analyzer.md`): notes the
  curator wrote itself (detected by their footer) are now tagged
  "[auto-generated by HomelabSage — background, not an operator
  constraint]" in the analyzer context, with the raw footer stripped. The
  prompt is updated so these can't trigger the versionlock / dependency
  severity-raise rules — those fire only on real operator notes, so the
  analyzer no longer inherits max confidence from its own earlier guess.
