...
- ✅ **CVE adapter (Trivy/Grype output → LLM context).** _Shipped: `src/homelabsage/cve.py` — runs `trivy image` first, falls back to `grype` when trivy is absent. Counts + top critical/high CVE IDs land in `Update.context.cve`. New prompt rule raises severity to critical when `counts.critical > 0`._
...
- ✅ **Parity-aware notification gate.** _Shipped: `src/homelabsage/parity.py` + `parity_gate:` config block. Push outputs (Telegram/Discord/Ntfy/Gotify) carry `is_push = True`; the engine skips them when `mdcmd status` or `/proc/mdstat` shows a resync/recovery/check/reshape. Notion (and the weekly digest) keep running. `homelabsage notify-pending --hours N` replays missed pushes manually._
...
- ✅ **Dependency-cascade detector (compose-aware).** _Shipped: `src/homelabsage/compose.py` builds a `DependencyGraph` from `sources.docker.compose_scan_paths`. Each Update gets `context.cascade.depends_on_me` + `network_neighbours`. Resolves both service name and `container_name` aliases so docker's runtime names map back correctly. New prompt rule surfaces up to 3 downstream services in `recommended_action`._
...
- ✅ **"What I see" diagnostics page.** _Shipped: `src/homelabsage/diagnostics.py` + `/diagnostics` HTML route + `/api/diagnostics` JSON. Verdicts: `tracked` / `floating_tag` / `no_repo` / `no_version` / `skipped_by_rule`. Probe is read-only (no outbound HTTP), cheap to refresh._
...
- ✅ **PUID/PGID change detector.** _Shipped as a prompt-rule + context flag: the docker plugin attaches `context.puid_pgid` when either env var is set, and a new analyzer rule fires only when the release notes mention permission/uid/gid terminology. Same shape as the renamed-env-var rule, no separate detector module needed._
