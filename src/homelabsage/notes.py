...
# A note file carrying this footer was written by HomelabSage's own curator,
# not by the operator. The analyzer must not treat its own prior guess as a
# REAL operator constraint (the versionlock / dependency rules in
# prompts/analyzer.md only apply to operator notes) — otherwise it inherits
# max confidence from its own homework. Mirrors `curator.helpers._FOOTER_RE`;
# `tests/test_notes_provenance.py` has a drift guard pinning the two together.
_CURATOR_FOOTER_RE = re.compile(
    r"<!--\s*curator:\s*[^@\s]+@[A-Za-z0-9]+\s*-->"
)

# Header suffix that flags an auto-generated section to the analyzer prompt.
_GENERATED_TAG = " [auto-generated by HomelabSage — background, not an operator constraint]"
...
class NotesProvider:
    ...
    def __init__(
        ...
        # (file, header_path, body, generated) — `generated` is True when the
        # file carries the curator footer, i.e. HomelabSage wrote it itself.
        self._sections: list[tuple[Path, str, str, bool]] = []
            ...
            if d.is_dir():
                for p in sorted(d.glob("**/*")):
                    if p.suffix.lower() in {".md", ".txt"} and p.is_file():
                        text = _read_text(p)
                        generated = _CURATOR_FOOTER_RE.search(text) is not None
                        for h, b in _sections(text):
                            if b:
                                self._sections.append((p, h, b, generated))
    ...
    def context_for(self, subject: str, keywords: list[str] | None = None) -> str:
        ...
        scored: list[tuple[int, Path, str, str, bool]] = []
        for f, h, b, gen in self._sections:
            s = _match_score(terms, h, b)
            if s > 0:
                scored.append((s, f, h, b, gen))
        ...
        # Then matched sections, best first. Auto-generated sections are
        # flagged in the header and have their curator footer stripped, so the
        # analyzer treats them as background rather than operator constraints.
        for _score, f, h, b, gen in scored:
            if budget <= 0:
                break
            tag = _GENERATED_TAG if gen else ""
            header = f"### {f.name} — {h}{tag}" if h else f"### {f.name}{tag}"
            body = _CURATOR_FOOTER_RE.sub("", b).rstrip() if gen else b
            chunk = f"{header}\n{body}"[: budget]
            parts.append(chunk)
            budget -= len(chunk)
