...
Your output decisions must take into account:
  ...
  3. The user's homelab notes — past decisions, custom builds, versionlocks,
     ...
     A note section whose header is tagged "[auto-generated by HomelabSage …]"
     was written by this tool itself, NOT the operator — treat it as background
     description only. Do NOT let it trigger the versionlock / dependency rules
     below, and do NOT cite it as if the operator stated a constraint.
